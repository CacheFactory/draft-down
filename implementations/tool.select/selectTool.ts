    ...
    if (event.key === 'Escape') {
      ...
      if (ids.length > 0) {
        ...
        for (const id of ids) {
          ...
          // Delete tape-measure guide lines (selectable construction lines)
          ...
          if (isFace) {
            // Remember the face's boundary edges — after deletion, those edges
            // might form a closed coplanar loop that should auto-become a new
            // face (e.g. delete the diagonal edge bisecting a quad → the four
            // outer edges should re-form a face).
        ...
        // After deletions, retry auto-face creation on edges that bordered the
        // removed geometry. If the remaining loop is closed + coplanar, a new
        // face is created; otherwise the call is a cheap no-op.
